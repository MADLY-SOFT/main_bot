...
async def top_players_levels(message: types.Message):
    ...
    last_top_usage[message.chat.id] = now
    cursor.execute('SELECT user_id, coins_win, coins_loss FROM users ORDER BY sms_main DESC') # Предполагаем, что sms_loss тоже есть в базе
    ...
    # Сортируем пользователей по уровню в убывающем порядке
    user_levels.sort(key=lambda item: item[1], reverse=True)

    top_message = "📊 <b>Топ пользователей по уровням:</b>\n\n"
    for rank, (user_id, level) in enumerate(user_levels[:10], start=1): # Берем топ-10
        user = await bot.get_chat(user_id)
        full_name = user.full_name 
...
async def trade_coins_for_stars(message: types.Message):
    try:
        ...
        if len(command_parts) == 1:  # Если введена только команда /trade
...
async def reset_logic():
    while True:
        ...
        if now.hour == 0 and now.minute == 0:
            ...
            # Если сегодня понедельник, сбрасываем и неделю
            if now.weekday() == 0:
            ...
            # Ждем минуту, чтобы не сработало повторно в ту же минуту
            await asyncio.sleep(61)
        
        await asyncio.sleep(30) # Проверка каждые 30 секунд

async def on_startup(_):
    # Запускаем фоновую задачу в текущем цикле событий
